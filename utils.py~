...
def calculate_black_regions(line):
    ...
    return filter_black_regions(start, final, count, critical_depth = 0.3 * len(line) )
...
def calculate_boundary(image, sample_rate=10, offset = 0., n_sigma=3.):
    ...
    for i in range(width):
        if i % sample_rate == 0:                
            ...
            start, depth, size = calculate_black_regions(image[:,i])
