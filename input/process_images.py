...
from skimage import  feature, color, data, restoration, exposure, img_as_float, img_as_uint, img_as_ubyte, util, measure
...
class Watershed(ProcessImage):
    ...
    def process(self, image):
        ...
        distance = ndimage.distance_transform_edt(rank.gradient(image, disk(3)))#sobel(image))

        local_max_coords = feature.peak_local_max(distance, min_distance = 20)
        print(tuple(local_max_coords.T))

        
        local_max_mask = np.zeros(distance.shape, dtype=bool)
        local_max_mask[tuple(local_max_coords.T)] = True #True
        print(np.where(local_max_mask == True))
        #local_max_mask = (image > np.mean(image))

        # local_max_mask = distance < 40
        local_max_mask = image > np.max(image)/2
        
        markers = np.ones(distance.shape, dtype=int)
        markers[distance < 10] = 1
        markers[distance > 25] = 2
 
        markers[tuple(local_max_coords.T)] = 0
        ...
        self.elevation_map = -distance #elevation_map
        ...
        # markers =  rank.gradient(image, disk(10)) < 20
        # markers = ndimage.label(markers)[0]
        self.markers = markers

        # local gradient (disk(2) is used to keep edges thin)
        gradient = -distance # rank.gradient(image, disk(2))
        self.elevation_map = sobel(image)#gradient
        # # Equalization
        self.thresh = threshold_otsu(image)
        # markers = image < self.thresh #np.max(image)/2 #(util.invert(image)/np.max(image)).astype(int)
        # markers = self.elevation_map > np.max(self.elevation_map)*6/8
        # markers = ndimage.label(markers)[0]
        self.labels = watershed(gradient, markers, mask=local_max_mask) #image>self.thresh)
    ...
    def plot(self, original_image):
        ...
        ax[0].imshow(self.elevation_map, cmap=plt.cm.nipy_spectral)

        ax[1].set_title('Image analysed')
        # ax[1].imshow(original_image, cmap=plt.cm.gray)
        ax[1].imshow(self.markers, cmap=plt.cm.nipy_spectral)




        ax[2].imshow(self.image_label_overlay, cmap=plt.cm.nipy_spectral)
        ax[2].contour(self.labels, [0.5], linewidths=1.2, colors='y')
        ax[2].set_title('Separated objects')

        ax[3].hist(self.image.ravel(), bins=256, histtype='step', color='black')
