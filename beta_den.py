...
Run `python3 beta_den.py --help` for the options to run this script. 

By default, if an option is chosen to run, the other steps in the pipeline steps will run as well. 
...
###--- Debugging ---###
If using the flag --show (-s) one will see plots of all the steps for each of the processing routines
...
One can test commands in the software_test_<date> directory above
say the test to analyse the already processed images in the directory

`python3 beta_den.py -s -a -i images_RemoveBakeliteBoundary_WhiteBackgroundRemoval_HistogramEquilization_OtsuThreshold`

`python3 beta_den.py -e -d AlphaBetaFraction_2021-12-03--14-52-06_images_RemoveBakeliteBoundary_WhiteBackgroundRemoval_HistogramEquilization_OtsuThreshold -s`
